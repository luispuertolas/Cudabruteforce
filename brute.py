...
def decode_permutation_kernel(results, base_idx, n, r):
    ...
    n: number of distinct characters (63)
    ...
    for position in range(r):
        # permutations of the (remaining-1) items for the remaining slots
        perms_of_rest = 1
...
def generate_permutations_gpu(r, print_flag, chunk_size=CHUNK_SIZE):
    """
    Enumerate all permutations of length r using the 63-character set, in chunks.
    ...
    # Append to file so each r adds more permutations
    with open("combinations.txt", "a", encoding="utf-8") as f_out:
        while current_offset < total_perms:
            # limit the chunk size if close to the end
...
if __name__ == "__main__":
    ...
    # Generate permutations for lengths 6 through 12
